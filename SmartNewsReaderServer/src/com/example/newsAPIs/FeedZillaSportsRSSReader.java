...
public class FeedZillaSportsRSSReader {
	private Logger log = Logger.getLogger(FeedZillaWordNewsRssReader.class.getName());
	private final String title = "FeedZilla Sports News";
	private static FeedZillaSportsRSSReader feedfZillaSports;
	private JSONObject data;
	private DBHelper db;

	private FeedZillaSportsRSSReader() {
		db = DBHelper.getDBInstance();
		data = db.getCompanyData(title);
	}

	public static void main(String[] args) throws JSONException {
		FeedZillaSportsRSSReader feedfZilla = FeedZillaSportsRSSReader.getInstance();
		//feedfZilla.checkForNewItems();
		JSONArray gcm_list = feedfZilla.db.getGCMListForNewsSource("2");
		GCMSender.sendGCMNotification(gcm_list, feedfZilla.db.getNewsItemInfo(1).toString());
	}
	
	public synchronized static FeedZillaSportsRSSReader getInstance() {
		if (feedfZillaSports == null) {
			feedfZillaSports = new FeedZillaSportsRSSReader();
		}
		return feedfZillaSports;
	}
	
	public void processfeedZillaSportsCronRequest(){
		try {
